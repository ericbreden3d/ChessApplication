		...
		for (int i = 1; i < 8 && found.b == false; i++) {
			...
			if (!getPathHelper(path, p, b, found, piece)) {
				if (found.b == true) {
					...
					if (b.isKing(piece)) {
						path.add(new Pair<Integer, Integer>(x + i + 1, y + i + 1));  // if threatened piece is opponent king then add one move past king
					}
					return path;
				}
				break;
		...
		for (int i = 1; i < 8 && found.b == false; i++) {
			...
			if (!getPathHelper(path, p, b, found, piece)) {
				if (found.b == true) {
					path.add(b.getPieceCoord(this));
					if (b.isKing(piece)) {
						path.add(new Pair<Integer, Integer>(x + i + 1, y - i - 1));
					}
					return path;
				}
				break;
